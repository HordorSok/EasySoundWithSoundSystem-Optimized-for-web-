...
namespace Lineri.SoundSystem
{
    ...
    public class EazySoundManager : MonoBehaviour
    {
        ...
        {
            // Ensure listAudio is not null
            if (listAudio == null)
            {
                Debug.LogError("listAudio is null.");
                return;
            }
            ...
            if (listAudio.Count > 0)
            {
                for (int i = 0; i < listAudio.Count; i++)
                {
                    ...
                    if (audio != null && !audio.Persist && !audio.Activated)
                    {
                        // Remove audio if it doesn't persist or isn't activated
                        DeleteAudio(audio, listAudio, ref i);
                    }
